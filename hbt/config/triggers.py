...
def add_triggers_2018(config: od.Config) -> None:
    ...
    Adds all triggers to a *config*. For the conversion from filter names to trigger bits, see
    https://github.com/cms-sw/cmssw/blob/master/PhysicsTools/NanoAOD/python/triggerObjects_cff.py.
    Triggers are taken from the Analysis node CMS AN-18-121
    Trigger Bits may still need to be corrected #TODO
    """
    config.x.triggers = od.UniqueObjectIndex(Trigger, [
        #
        # single electron
        ...
        Trigger(
            ...
            applies_to_dataset=(lambda dataset_inst: (
                dataset_inst.is_mc or (dataset_inst.is_data and dataset_inst.x.era >= "B")
            )),
            tags={"cross_trigger", "cross_e_tau", "channel_e_tau"},
        ),
        ...
        #
        Trigger(
            name="HLT_VBF_DoubleLooseChargedIsoPFTau20_Trk1_eta2p1",
            id=602,
            legs=[
                TriggerLeg(
                    pdg_id=15,
